...
- ✅ Автоматическая проверка всех сообщений в чате на мат
- ✅ Удаление сообщений с вероятностью мата ≥ 0.7
- ✅ Использование ML-модели (SVM) для определения контекста
- ✅ Поддержка русского языка и транслитерации
- ✅ Ежедневная статистика работы бота (отправляется админам в 21:00)
- ✅ Работа только в указанных чатах (безопасность)
...
Статистика отправляется всем админам (указанным в `ADMIN_IDS`) **ежедневно в 21:00** по московскому времени. После отправки статистика автоматически сбрасывается.
...
Бот работает **только** в чатах, указанных в `ALLOWED_CHAT_IDS`. Сообщения из других чатов полностью игнорируются. Это гарантирует, что бот не будет работать в чужих группах без вашего разрешения.

## Примечания

- Бот работает **только** в чатах, указанных в `ALLOWED_CHAT_IDS`
...
- Чтобы добавить новый чат, добавьте его chat_id в `ALLOWED_CHAT_IDS` через запятую и перезапустите бота
- Статистика отправляется автоматически в 21:00 ежедневно всем админам из `ADMIN_IDS`
