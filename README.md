...
EXEMPT_SENDER_CHAT_IDS=-1001234567890,-1009876543210
```
   ...
   - `EXEMPT_SENDER_CHAT_IDS` - необязательно. ID канала и/или чата через запятую; сообщения от этих отправителей (посты из привязанного канала, посты «от имени группы») не удаляются. Остальные сообщения с sender_chat по-прежнему модерируются.
